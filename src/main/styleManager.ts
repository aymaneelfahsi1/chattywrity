    ...
    async getActiveAppInfo(): Promise<ActiveAppInfo> {
        try {
            ...
            if (window?.owner?.path) {
                this.currentApp = window.owner.name;
                const iconDataUrl = await this.extractAppIcon(window.owner.path);
                return {
        ...
        return { name: '', iconDataUrl: undefined };
    }

    private async extractAppIcon(exePath?: string): Promise<string | undefined> {
